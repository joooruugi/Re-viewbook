 ...
 */
@WebServlet("/fundingread")
public class FundingReadServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;

	public FundingReadServlet() {
		super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (updateCount < 1) {// 조회수 +1 실패시 목록으로
			...
			if(vo == null) { //로그인이 되지 않은 상황 -> 로그인 해야만 글 읽기 가능
				...
				request.getRequestDispatcher("WEB-INF/view/user/funding/fundingread.jsp").forward(request, response);
			}
