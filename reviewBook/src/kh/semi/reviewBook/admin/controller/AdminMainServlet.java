...
public class AdminMainServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		System.out.println("관리자 메인 서블릿 확인");
		AdminVo avo = (AdminVo)request.getSession().getAttribute("ssAdminVo");
		if(avo == null) {
			response.sendRedirect("adlogin");
			return;
		}else {
			String adId = avo.getAdId();
			avo.setAdId(adId);
		}
		ArrayList<AdminVo> alist = service.AdminList(avo.getAdId());
		request.getRequestDispatcher("WEB-INF/view/admin/main/admain.jsp").forward(request, response);
	}
